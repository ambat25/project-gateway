...
export const ProjectCard = ({
  ...
  const getTotal = () =>
    Number.parseFloat(
      data?.reduce(
        (acc, cur) => Number.parseFloat(acc) + Number.parseFloat(cur.amount),
        0
      )
    );
  if (!data) return null;
  return (
    <Box pad="0 0 5px">
      <Card pad="17px 19px" cursor="pointer" onClick={onToggle}>
        <ProjectCardWrapper>
          <span>{title}</span>
          <span>Total: {numberWithCommas(getTotal().toFixed(2))} USD</span>
        </ProjectCardWrapper>
      </Card>
      {isOpened && (
              ...
              {data
                ...
                ?.map((row, index) => (
                  <tr key={index}>
                    ...
                    <TD>{indexer?.[row?.[subKey]]?.name}</TD>
                    <TD textAlign="center">{row.paymentId}</TD>
                    <TD textAlign="right">
